class StoriesManager {
  constructor() {
    ...
    // Инициализация вызывается сразу при создании экземпляра
    this.init();
  }
  ...
  init() {
    ...
    // Ищем данные в глобальном объекте, который должен загрузиться до создания этого класса
    if (window.storiesData) {
        this.placeData = window.storiesData[this.placeId];
    }

    if (!this.placeData) {
      console.error(`Данные для места "${this.placeId}" не найдены. Убедитесь, что категория (${urlParams.get('cat')}) загружена.`);
      // Можно показать ошибку пользователю
      this.container.innerHTML = '<div style="color: white; text-align: center; padding-top: 50%; font-size: 18px;">Ошибка загрузки данных.<br>Данные для этого места не найдены.</div>';
      return;
    }
...
// ==========================================
// ЛОГИКА ДИНАМИЧЕСКОЙ ЗАГРУЗКИ ДАННЫХ
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!category || !placeId) {
        console.error("Ошибка загрузки: Не указаны параметры cat или place в URL.");
        document.body.innerHTML = '<div style="color:white; text-align:center; padding-top:50%;">Ошибка загрузки страницы.<br>Не выбрано место.</div>';
        return;
    }

    // Сопоставление категорий с файлами данных
    const filesMap = {
        'religious': 'js/stories_data/stories_religious.js',
        'museums': 'js/stories_data/museums_stories.js',
        'nature': 'js/stories_data/nature_stories.js',
        'food': 'js/stories_data/food_stories.js'
        // Если у вас другие названия категорий, добавьте их сюда
    };

    const scriptSrc = filesMap[category];

    if (!scriptSrc) {
        console.error(`Файл для категории "${category}" не найден в списке.`);
        document.body.innerHTML = '<div style="color:white; text-align:center; padding-top:50%;">Ошибка: Категория не найдена.</div>';
        return;
    }
    ...
    script.charset = 'utf-8'; // Для корректного отображения кириллицы

    script.onload = () => {
        console.log(`Файл данных "${scriptSrc}" загружен.`);
        
        // Проверяем, загрузились ли данные и есть ли нужное место
        if (window.storiesData && window.storiesData[placeId]) {
            // Данные есть, запускаем приложение
            window.storiesManager = new StoriesManager();
        } else {
            console.error(`Место с ID "${placeId}" не найдено в файле "${scriptSrc}"`);
            document.body.innerHTML = '<div style="color:white; text-align:center; padding-top:50%;">Ошибка: Контент для этого места не найден.</div>';
        }
    };

    script.onerror = () => {
        console.error(`Не удалось загрузить файл: ${scriptSrc}`);
        document.body.innerHTML = '<div style="color:white; text-align:center; padding-top:50%;">Ошибка соединения.<br>Не удалось загрузить данные.</div>';
    };

    // Добавляем скрипт в документ для начала загрузки
